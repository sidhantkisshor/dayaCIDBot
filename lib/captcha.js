...
export async function handleCallbackQuery(callbackQuery, res) {
  ...
  if (!pending) {
    // State was lost (e.g. serverless cold start). Do NOT blindly unmute — that
    // would skip the answer check and let any button press through. Re-issue a
    // fresh captcha instead: preserves the answer requirement, and the bounded
    // mute guarantees the user is never stuck forever.
    await answerCallbackQuery(id, 'Session expired — please solve the new check below.');
    await startVerification(chatId, from);
    return res.status(200).json({ ok: true });
  }
