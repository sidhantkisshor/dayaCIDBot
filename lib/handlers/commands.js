...
async function handleBan(message) {
  ...
  if (message.reply_to_message) {
    ...
    // Don't ban bots or admins
    if (targetUser.is_bot) {
      await bot.sendMessage(chat.id, "❌ Cannot ban a bot.");
      return;
    }

    if (await isAdmin(chat.id, targetUser.id)) {
      await bot.sendMessage(chat.id, "❌ Cannot ban an admin.");
      return;
    }
    ...
    try {
      ...
      await bot.sendMessage(
        chat.id,
        `🚫 User ${targetUser.first_name} has been permanently banned.`
      );
    } catch (error) {
      await bot.sendMessage(chat.id, `❌ Failed to ban user: ${error.message}`);
    }
  } else {
    // Check for mention
    const mention = message.entities?.find(e => e.type === 'text_mention');
    if (!mention) {
      await bot.sendMessage(chat.id, "ℹ️ Usage: /ban @user or reply to their message");
      return;
    }
...
async function handleUnban(message) {
  ...
  const mention = message.entities?.find(e => e.type === 'text_mention');
  if (!mention) {
    await bot.sendMessage(chat.id, "ℹ️ Usage: /unban @user");
    return;
  }
  ...
  try {
    await bot.unbanChatMember(chat.id, targetUser.id);
    await bot.sendMessage(
      chat.id,
      `✅ User ${targetUser.first_name} has been unbanned.`
    );
  } catch (error) {
    await bot.sendMessage(chat.id, `❌ Failed to unban user: ${error.message}`);
  }
}
