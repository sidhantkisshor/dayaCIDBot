...
import { sendMessage, banUser, unbanUser, isAdmin, getBotMember, logToAdmin } from './telegram.js';
    ...
    case '/ban':
      if (await isAdmin(chatId, userId)) {
        if (message.reply_to_message) {
          ...
          if (banResult?.ok) {
            await sendMessage(chatId, `🚫 Banned ${targetUser.first_name || 'user'}`, false, threadId);
      ...
      if (await isAdmin(chatId, userId)) {
        if (message.reply_to_message) {
          ...
          if (result?.ok) {
            ...
            await sendMessage(chatId, `✅ Unbanned ${targetUser.first_name || 'user'}`, false, threadId);
    ...
    case '/trust':
      if (await isAdmin(chatId, userId)) {
        if (message.reply_to_message) {
          ...
          await deleteWarnings(chatId, targetUser.id);
          clearReports(chatId, targetUser.id);
          await sendMessage(chatId, `✅ ${targetUser.first_name || 'User'} is now trusted`, false, threadId);
        } else {
    ...
    case '/untrust':
      if (await isAdmin(chatId, userId)) {
        if (message.reply_to_message) {
          ...
          await sendMessage(chatId, `⚠️ ${targetUser.first_name || 'User'} is no longer trusted`, false, threadId);
        } else {
