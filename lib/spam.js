...
import {
  FLOOD_WINDOW_MS, FLOOD_THRESHOLD,
...
export const SPAM_PATTERNS = [
  ...
  // Chinese/CJK Adult Content & Spam
  /[乱伦色情淫荡裸体做爱性爱嫖娼卖淫]/,
  /#[^\s#]*[乱伦色情淫荡裸聊约炮]/,
  /点击查看|完整视频|免费观看|在线观看|立即观看/,
  /全网资源|资源搜索|资源群|福利群|色群/,
  /约炮|裸聊|一夜情|小姐姐.*视频|视频.*小姐姐/,
  /加群|加我|私聊|上车|开车|老司机/,
  /[\u4e00-\u9fff].*https?:\/\/[^\s]+.*[\u4e00-\u9fff]/,

  // Korean/Japanese Adult Content Spam
  /무료.*영상|영상.*무료|성인.*방송|카지노|도박/,
  /無料.*動画|動画.*無料|アダルト|出会い系/,

  // Multilingual porn/adult domains
  /\b(pornhub|xvideos|xhamster|xnxx|redtube|youporn)\b/i,

  // Generic CJK spam signals: hashtag-heavy + CJK text
  /(#[\u4e00-\u9fff\uac00-\ud7af\u3040-\u309f\u30a0-\u30ff]+\s*){3,}/,
];
...
export async function isSpam(text, userId, chatId, username, message = null, hasUsername = true) {
  // Skip trusted users (before any tracking so their activity isn't recorded).
  ...
  // Behavior analysis runs for EVERY message, including captionless media
  // (stickers, photos with no caption), so media floods are still caught.
  ...
  // Text-based checks only apply when there is text/caption. For captionless
  // media, the behavior score above is the whole verdict.
  if (!text) {
    return { isSpam: score >= getThreshold('SPAM_THRESHOLD'), score, reasons };
  }
