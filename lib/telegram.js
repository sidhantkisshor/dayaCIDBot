...
// Set webhook with allowed_updates. When secretToken is provided, Telegram will
// send it back in the X-Telegram-Bot-Api-Secret-Token header on every request,
// letting the webhook verify the call really came from Telegram.
export async function setWebhook(url, allowedUpdates, secretToken = null) {
  const body = {
    url: url,
    allowed_updates: allowedUpdates
  };
  if (secretToken) body.secret_token = secretToken;
  return await callTelegram('setWebhook', body);
}

// Approve a pending join request (for groups with "approve new members" on).
