...
// Trading-specific scam patterns (legacy - kept for backward compatibility)
const LEGACY_SCAM_PATTERNS = [
  // Guaranteed profits
...
export async function checkScam(message, userData) {
  ...
  // Use comprehensive pattern checking
  const patternCheck = checkPatterns(content, SPAM_PATTERNS);

  // Focus on scam-specific categories
  const scamCategories = ['CRYPTO_SCAMS', 'SIGNAL_SCAMS', 'BOT_SCAMS', 'RECOVERY_SCAMS',
                          'GIVEAWAY_SCAMS', 'LOAN_SCAMS', 'MLM_SCHEMES'];

  const scamMatches = patternCheck.matches.filter(m => scamCategories.includes(m.category));
  if (scamMatches.length > 0) {
    score += scamMatches.reduce((sum, m) => sum + m.score, 0);
    console.log(`[SCAM] Detected ${scamMatches.length} scam patterns:`,
      scamMatches.map(m => m.category).join(', '));
  }
  
  // 1. Additional legacy pattern check (for specific trading scams)
  for (const pattern of LEGACY_SCAM_PATTERNS) {
    if (pattern.regex.test(content)) {
      score += Math.min(pattern.score, 2); // Reduce weight since we're double-checking
    }
  }
...
async function isSuspiciousUrl(url) {
  ...
  if (result.suspicious) {
    console.log(`[SCAM URL] ${result.reason}: ${url}`);
  }
