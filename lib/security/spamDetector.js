...
import { SPAM_PATTERNS, checkPatterns, EMOJI_SPAM, SUSPICIOUS_USERNAMES } from './patterns.js';
...
export async function checkSpam(message, userData) {
  ...
  // Check against comprehensive spam patterns
  const patternCheck = checkPatterns(content, SPAM_PATTERNS);
  score += Math.min(patternCheck.totalScore, 8); // Cap pattern score at 8

  if (patternCheck.matches.length > 0) {
    console.log(`[PATTERN MATCH] Found ${patternCheck.matches.length} spam patterns:`,
      patternCheck.matches.map(m => m.category).join(', '));
  }
  ...
  // 6.1. Check username patterns
  const username = from.username || '';
  const displayName = from.first_name + (from.last_name || '');

  for (const pattern of SUSPICIOUS_USERNAMES.patterns) {
    if (pattern.test(username) || pattern.test(displayName)) {
      score += SUSPICIOUS_USERNAMES.score;
      console.log(`[SPAM] Suspicious username/display name: ${username || displayName}`);
      break;
    }
  }

  // 7. Emoji spam detection
  const emojiMatches = content.match(/[\u{1F300}-\u{1F9FF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}]/gu) || [];
  const emojiCount = emojiMatches.length;

  // Check emoji patterns
  for (const emojiPattern of EMOJI_SPAM.patterns) {
    if (emojiPattern.regex.test(content)) {
      score += emojiPattern.score;
      console.log('[SPAM] Excessive emoji pattern detected');
    }
  }
  ...
  if (content.length > 0) {
    ...
    if (emojiRatio > EMOJI_SPAM.maxEmojiRatio) {
      score += EMOJI_SPAM.excessiveEmojiScore;
      console.log(`[SPAM] High emoji density: ${Math.round(emojiRatio * 100)}%`);
    }
  }
